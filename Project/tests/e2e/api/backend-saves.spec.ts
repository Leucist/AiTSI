...
test.describe('Backend Saves and API', () => {
    ...
    test('[R8] Saving from Pause menu hits backend API successfully', async ({ page }) => {
    ...
    test('[R8] Saved game updates the SQLite DB correctly', async ({ page }) => {
        await startNewGameSpecific(page);
    ...
    test('[R9] Missing auth token results in 401 error', async ({ request }) => {
    ...
    test('[R9] Incorrect auth token results in 401 error', async ({ request }) => {
        const res = await request.post(`${BACKEND_URL}/api/save`, {
            data: { levelIndex: 0, score: 0, coinsCollected: [] },
    ...
    test('[R9] Backend correctly responds with hasSave = false when empty', async ({ request }) => {
    ...
    test('[R9] Backend correctly serves the latest save data', async ({ page, request }) => {
        await startNewGameSpecific(page);
        await setGamePlayerPos(page, 245, 245);
        await tick(page, 2);
        await page.keyboard.press('Escape');
    ...
    test('[R9] Reloading page correctly fetches save data from Backend via API', async ({ page }) => {
    ...
    test('[R9] Continuing game fetches level index and score properly from Backend', async ({ page }) => {
    ...
    test('[R8] Backend properly overrides duplicate saves for same user', async ({ page }) => {
        await startNewGameSpecific(page);
        await page.keyboard.press('Escape');
        await expect(page.getByTestId('btn-save-progress')).toBeVisible();
        await Promise.all([
    ...
    test('[R9] Starting a new adventure from Home Screen ignores existing save', async ({ page }) => {
        db.prepare('INSERT OR REPLACE INTO save_games (user_id, level_index, score, coins_collected) VALUES (1, 1, 80, \'[]\')').run();
        await page.reload();
        await expect(page.getByTestId('btn-continue-journey')).toBeVisible();
