...
test.describe('UI Interactions and Boundaries', () => {
    ...
    test('[R6] Player falling out of bounds triggers Game Over', async ({ page }) => {
    ...
    test('[R3] Player respects left boundary', async ({ page }) => {
        await startNewGameSpecific(page);
    ...
    test('[R1][R2] Player can move left and jump simultaneously', async ({ page }) => {
        await startNewGameSpecific(page);
        await tick(page, 40);
        await page.keyboard.down('ArrowLeft');
    ...
    test('[R1][R2] Player can move right and jump simultaneously', async ({ page }) => {
        await startNewGameSpecific(page);
        await tick(page, 40);
    ...
    test('[R7] HUD shows correct level initially as STAGE 1 / 3', async ({ page }) => {
        await startNewGameSpecific(page);
        await expect(page.getByTestId('hud-level')).toHaveText('STAGE 1 / 3');
    });

    test('[R10] Pause menu appears when Escape is pressed while playing', async ({ page }) => {
        await startNewGameSpecific(page);
    ...
    test('[R10] Pause menu resumes game when Resume is clicked', async ({ page }) => {
        await startNewGameSpecific(page);
        await page.keyboard.press('Escape');
        await expect(page.getByText('PAUSED')).toBeVisible();
    ...
    test('[R7] HUD shows 0 score after game over and restart', async ({ page }) => {
        await startNewGameSpecific(page);
    ...
    test('[R6] Game Over state correctly resets game mechanics when starting new game', async ({ page }) => {
        await startNewGameSpecific(page);
        await setGamePlayerPos(page, 50, 900);
        await tick(page, 5);
    ...
    test('[R6] Restarting the game from Game Over leaves no residual coins', async ({ page }) => {
        await startNewGameSpecific(page);
        await setGamePlayerPos(page, 245, 245);
        await tick(page, 5);
        await setGamePlayerPos(page, 50, 900);
