...
test.describe('Game Logic and Progression', () => {
    ...
    test('[R7] Initial score begins at 0', async ({ page }) => {
        await startNewGameSpecific(page);
        const state = await getGameState(page);
        expect(state.score).toBe(0);
    });

    test('[R7] Initial coins collected begins empty', async ({ page }) => {
        await startNewGameSpecific(page);
        const state = await getGameState(page);
        expect(state.coinsCollectedArr.length).toBe(0);
    });

    test('[R4] Player touching a coin adds it to collected Set', async ({ page }) => {
        await startNewGameSpecific(page);
    ...
    test('[R4] Player touching a coin increments the score by +10', async ({ page }) => {
        await startNewGameSpecific(page);
        await setGamePlayerPos(page, 245, 245);
        await tick(page, 5);
        const state = await getGameState(page);
    ...
    test('[R7] HUD score updates immediately after coin collect', async ({ page }) => {
        await startNewGameSpecific(page);
        await setGamePlayerPos(page, 245, 245);
        await tick(page, 5);
    ...
    test('[R4] Collecting same coin space twice does not double count', async ({ page }) => {
        await startNewGameSpecific(page);
        await setGamePlayerPos(page, 245, 245);
        await tick(page, 5);
    ...
    test('[R4] Multiple coins correctly stack scores', async ({ page }) => {
        await startNewGameSpecific(page);
        await setGamePlayerPos(page, 245, 245);
        await tick(page, 4);
    ...
    test('[R5] Reaching the end door triggers Level Complete overlay', async ({ page }) => {
        await startNewGameSpecific(page);
    ...
    test('[R5] Level transition to Level 2 sets correct levelIndex', async ({ page }) => {
        await startNewGameSpecific(page);
        await setGamePlayerPos(page, 755, 335);
        await tick(page, 5);
    ...
    test('[R10] Navigating to the end of the final level triggers Win Overlay', async ({ page }) => {
        await startNewGameSpecific(page);
