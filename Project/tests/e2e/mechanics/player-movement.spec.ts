...
test.describe('Player Movement Mechanics', () => {
    ...
    test('[R10] Player begins at expected start position', async ({ page }) => {
    ...
    test('[R1] Player moves right when ArrowRight is held', async ({ page }) => {
        await startNewGameSpecific(page);
    ...
    test('[R1] Player moves left when ArrowLeft is held', async ({ page }) => {
        await startNewGameSpecific(page);
        await page.keyboard.down('ArrowRight');
        await tick(page, 5);
        await page.keyboard.up('ArrowRight');
    ...
    test('[R2] Player executes jump with ArrowUp when grounded', async ({ page }) => {
        await startNewGameSpecific(page);
        await tick(page, 40);
    ...
    test('[R3] Player gets affected by gravity pulling down', async ({ page }) => {
        await startNewGameSpecific(page);
    ...
    test('[R1] Player facing direction updates right', async ({ page }) => {
        await startNewGameSpecific(page);
        await page.keyboard.down('ArrowRight');
        await tick(page, 5);
        const state = await getGameState(page);
    ...
    test('[R1] Player facing direction updates left', async ({ page }) => {
        await startNewGameSpecific(page);
        await page.keyboard.down('ArrowLeft');
        await tick(page, 5);
        const state = await getGameState(page);
    ...
    test('[R2] Player cannot jump when in air', async ({ page }) => {
        await startNewGameSpecific(page);
        await setGamePlayerPos(page, 50, 100);
        await page.keyboard.down('ArrowUp');
        await tick(page, 3);
    ...
    test('[R3] Player is grounded when standing on a platform', async ({ page }) => {
        await startNewGameSpecific(page);
        await tick(page, 60);
        const state = await getGameState(page);
    ...
    test('[R1] Player stops completely if no movement keys are pressed', async ({ page }) => {
        await startNewGameSpecific(page);
        await page.keyboard.down('ArrowRight');
        await tick(page, 5);
        await page.keyboard.up('ArrowRight');
